...
// Schema de Medicação Refatorado (Apenas uma versão correta)
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    // ✅ CORREÇÃO: Cria o documento de medicação com a nova estrutura correta
...
app.post('/api/dailylog/track', autenticar, async (req, res) => {
    try {
        ...
        res.json(updatedLog);
    } catch (error) { res.status(500).json({ message: "Erro ao registar consumo." }); }
});
