...
const User = require('../models/userModel'); // CORRIGIDO
...
exports.gerarConvite = async (req, res) => {
  try {
    ...
    res.status(201).json({ message: 'Convite gerado com sucesso!', codigo: convite.codigo, url: urlConvite });
  } catch (error) {
...
exports.aceitarConvite = async (req, res) => {
    ...
    try {
        ...
        const paciente = await User.findById(pacienteId); // CORRIGIDO
